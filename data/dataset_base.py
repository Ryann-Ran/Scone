...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def set_sequence_status(self):
        sequence_status = dict(
            ...
            ###### Scone
            packed_instruc_indexes      = list(),
            patchified_vit_token_shapes = list(),
            ###### Scone
        )
    ...
    def to_tensor(self, sequence_status):
        data = dict(
            ...
            ###### Scone
            packed_instruc_indexes=torch.tensor(sequence_status['packed_instruc_indexes']),
            ###### Scone
        )
        ...
        if len(sequence_status['packed_vit_tokens']) > 0:
            ...
            ###### Scone
            data['patchified_vit_token_shapes'] = sequence_status['patchified_vit_token_shapes']
            ###### Scone
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        for item in sequence_plan:
            ...
            if item['type'] == 'text':
                ...
                ###### Scone
                sequence_status['packed_instruc_indexes'].extend(range(curr, curr + len(shifted_text_ids)))
                ###### Scone
                if item['loss'] == 1:
                ...
                ###### Scone
                sequence_status['packed_instruc_indexes'].append(curr)
                ###### Scone
                ...
                # preprocess image
                ###### Scone
                vit_tokens, vit_token_shape = patchify(image_tensor, self.data_config.vit_patch_size, return_hw=True)
                sequence_status['patchified_vit_token_shapes'].append(vit_token_shape)
                ###### Scone
...
class SimpleCustomBatch:
    def __init__(self, batch):
        ...
        ###### Scone
        self.packed_instruc_indexes = data["packed_instruc_indexes"]
        ###### Scone
        ...
        if "packed_vit_tokens" in data.keys():
            ...
            ###### Scone
            self.patchified_vit_token_shapes = data["patchified_vit_token_shapes"]
            ###### Scone
    ...
    def pin_memory(self):
        ...
        ###### Scone
        self.packed_instruc_indexes = self.packed_instruc_indexes.pin_memory()
        ###### Scone
    ...
    def cuda(self, device):
        ...
        ###### Scone
        self.packed_instruc_indexes = self.packed_instruc_indexes.to(device)
        ###### Scone
    ...
    def to_dict(self):
        data = dict(
            ...
            ###### Scone
            packed_instruc_indexes = self.packed_instruc_indexes,
            ###### Scone
        )
        ...
        if hasattr(self, 'packed_vit_tokens'):
            ...
            ###### Scone
            data['patchified_vit_token_shapes'] = self.patchified_vit_token_shapes
            ###### Scone

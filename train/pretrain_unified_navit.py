...
class TrainingArguments:
    ...
    ###### Scone
    freeze_mlp_und: bool = field(
        default=False,
        metadata={"help": "Freeze the weights of the MLP used for understanding (no gradient updates)."}
    )
    freeze_mlp_gen: bool = field(
        default=False,
        metadata={"help": "Freeze the weights of the MLP used for generation (no gradient updates)."}
    )
    freeze_llm_und: bool = field(
        default=False,
        metadata={"help": "Freeze the weights of the language model used for understanding (no gradient updates)."}
    )
    freeze_llm_gen: bool = field(
        default=False,
        metadata={"help": "Freeze the weights of the language model used for generation (no gradient updates)."}
    )
    use_semantic_mask: bool = field(
        default=False,
        metadata={"help": "Enable understanding bridge strategy algorithm."}
    )
    ###### Scone
...
def main():
    ...
    ###### Scone
    llm_config.use_semantic_mask = training_args.use_semantic_mask
    ###### Scone
    if training_args.finetune_from_hf:
    ...
    ###### Scone
        ...
        if training_args.freeze_llm_und:
            for name, param in model.language_model.model.named_parameters():
                if "moe_gen" in name or "embed_tokens" in name:
                    continue
                param.requires_grad = False
                logger.info(f"Freeze LLM: {name}")

        if training_args.freeze_llm_gen:
            logger.info(f"Freeze LLM-GEN")
            for name, param in model.language_model.model.named_parameters():
                if "moe_gen" in name:
                    param.requires_grad = False
                    logger.info(f"Freeze LLM: {name}")
    ...
    # Freeze connector, vae2llm, llm2vae
    if training_args.freeze_mlp_und:
        for name, param in model.named_parameters():
            for mlp_name in ['connector']:
                if mlp_name in name:
                    param.requires_grad = False
                    logger.info(f"Freeze mlp: {name}")

    if training_args.freeze_mlp_gen:
        for name, param in model.named_parameters():
            for mlp_name in ['vae2llm', 'llm2vae']:
                if mlp_name in name:
                    param.requires_grad = False
                    logger.info(f"Freeze mlp: {name}")
    ###### Scone

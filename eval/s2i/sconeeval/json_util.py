...
def mllm_output_to_int(input_string, give_up_parsing=False):
    """
    Parse MLLM output string into an integer (1 or 0).
    Model output examples:
        "1"  -> Subject is present
        "0"  -> Subject is absent
    """
    # Handle edge cases
    if input_string is None:
        if give_up_parsing:
            return None
        raise Exception("Input string is None.")
    ...
    # 2. Search for isolated digit if string contains other characters
    m = re.search(r'\b[01]\b', s)
    if m:
        return int(m.group())

    # 3. All parsing attempts failed
    if give_up_parsing:
        return None
    
    raise Exception(f"Failed to extract 0/1 integer from output: {input_string=}")